...
import { youtubeAPI, YouTubePlaylist, YouTubeVideo } from '../lib/api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const playlistResponse = await youtubeAPI.getPlaylists(6);
        console.log('Playlists response:', playlistResponse);
        ...
        const videoResponse = await youtubeAPI.getChannelVideos(1);
        console.log('Videos response:', videoResponse);
        
        if (videoResponse.success && videoResponse.videos.length > 0) {
          setLatestVideo(videoResponse.videos[0]);
        }
      } catch (error) {
